...
- 用户说"查顾客XXX" → tool: query_customer
- 用户说"查退款" → tool: query_refunds
...
- **tool**: 直接调用预定义工具函数，精确执行。当用户的问题有对应的 tool 名时，必须使用 tool
  - "查退款" → tool: query_refunds
  - "批准退款" → tool: refund_approve
  - "查顾客XXX" → tool: query_customer
  - "查库存" → tool: query_inventory
- **nl2sql**: 需要 LLM 生成 SQL 查询数据库。当没有对应的 tool 时使用
  - "本月营业额趋势" → nl2sql（没有 query_revenue_trend 对应的精确查询）
